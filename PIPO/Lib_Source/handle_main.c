#include "astbb_srbn.h"



char alg_name[256] = "FLY";
SUB_WRD_t sbox[256] = {  };
GEN_CNT_t off[8] = { 7, 6, 5, 4, 3, 2, 1, 0};

SRBPN_INFO_t cip =
{
	alg_name,
	8,
	256,
	sbox,

	8,
	off,
};



PROB_t rst;
PROB_t rsts[100];

void main(void)
{

	int target_round;
	target_round = 8;
	NUM_TRAIL_t r;
	Prep_Dif_Trail_Searching(&cip);
	Computation_LS_of_DDT();
	int i;
	
	for (i = 1; i <= target_round; i++)
	{
		Best_Trail_Prob_Only(&rst, i, rsts);
		rsts[i] = rst;
	}
	//rsts[target_round] = -63;
	
	for (i = 1; i <= target_round; i++)
	{
		printf("%0.1f", rsts[i]);
	}
	
	printf("\nFIN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");

	
}

