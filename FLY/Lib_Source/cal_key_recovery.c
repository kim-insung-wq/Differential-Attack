...
WEIGHT_t Cal_Key_Recovery_Comp_11R(ANA_STATE_t _1r_SUB_IN, ANA_STATE_t _nr_SUB_OUT, WEIGHT_t GIVEN_PROB, int num_of_right_pair, double c_e)
{
	int i;
...
WEIGHT_t Cal_Key_Recovery_Comp_12R(ANA_STATE_t _1r_SUB_IN, ANA_STATE_t _nr_SUB_OUT, WEIGHT_t GIVEN_PROB, int num_of_right_pair, double c_e)
{
	int i;
	BitPattern64 _2r_BP = { 0 };
	BitPattern64 _1r_BP = { 0 };
	...
	PERM(DIFF_OUT, _nr_SUB_OUT);
	INV_PERM(DIFF_IN, _1r_SUB_IN);
	int num_of_fwd_guessed_sboxes_2r = NUM_SBOX_IN_A_STATE;
	int num_of_fwd_guessed_sboxes_1r = NUM_SBOX_IN_A_STATE - Count_Trivial_Sbox(DIFF_IN);
	WEIGHT_t d_in = SBOX_BIT_SIZE * num_of_fwd_guessed_sboxes_2r;
	int num_of_bwd_guessed_sboxes_1r = NUM_SBOX_IN_A_STATE - Count_Trivial_Sbox(DIFF_OUT);
	int num_of_bwd_guessed_sboxes_2r = NUM_SBOX_IN_A_STATE;
	int num_of_bwd_nontrivial_sboxes_2r = NUM_SBOX_IN_A_STATE - Cal_2r_BP(_nr_SUB_OUT, &_2r_BP);
	double ciphertext_filtering_ratio = SBOX_BIT_SIZE * (NUM_SBOX_IN_A_STATE - num_of_bwd_nontrivial_sboxes_2r);
	WEIGHT_t tot_num_of_guessed_sboxes = num_of_fwd_guessed_sboxes_2r + num_of_fwd_guessed_sboxes_1r + num_of_bwd_guessed_sboxes_1r + num_of_bwd_guessed_sboxes_2r;
	WEIGHT_t guessing_2r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t guessing_1r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t guessing_1r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t guessing_2r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t filtering_2r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t filtering_1r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t filtering_1r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t filtering_2r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };
	WEIGHT_t w_tot_time_comp;
	...
	for (i = 0; i < num_of_fwd_guessed_sboxes_2r; i++)
	{
		guessing_2r_fwd[i] = SBOX_BIT_SIZE;
		filtering_2r_fwd[i] = Count_Trivial_Sbox(DIFF_IN);
	}
...
#if defined DEBUG
	printf("\n");
	printf("\guessing_2r_fwd : ");
	for (i = 0; i < num_of_fwd_guessed_sboxes_2r; i++)
	{
		printf("%f, ", guessing_2r_fwd[i]);
	}
	printf("\n");

	printf("\filtering_2r_fwd : ");
	for (i = 0; i < num_of_fwd_guessed_sboxes_2r; i++)
	{
		printf("%f, ", filtering_2r_fwd[i]);
	}
	printf("\n");
	...
	Element comp_arry_2r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD];
	Element comp_arry_1r_fwd[NUM_SBOX_IN_A_STATE_THRESHOLD];
	Element comp_arry_1r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD];
	Element comp_arry_2r_bwd[NUM_SBOX_IN_A_STATE_THRESHOLD];
	...
	for (i = 0; i < NUM_SBOX_IN_A_STATE_THRESHOLD; i++)
	{
		if (i < num_of_fwd_guessed_sboxes_2r)
		{
			comp_arry_2r_fwd[i].value = guessing_2r_fwd[i] - filtering_2r_fwd[i];
		}
		else
		{
			comp_arry_2r_fwd[i].value = 100000;
		}
		comp_arry_2r_fwd[i].index = i;
	}
	...
	qsort(comp_arry_2r_fwd, NUM_SBOX_IN_A_STATE_THRESHOLD, sizeof(Element), compare);
	qsort(comp_arry_1r_fwd, NUM_SBOX_IN_A_STATE_THRESHOLD, sizeof(Element), compare);
	qsort(comp_arry_1r_bwd, NUM_SBOX_IN_A_STATE_THRESHOLD, sizeof(Element), compare);
	qsort(comp_arry_2r_bwd, NUM_SBOX_IN_A_STATE_THRESHOLD, sizeof(Element), compare);

	WEIGHT_t sequence[4 * NUM_SBOX_IN_A_STATE_THRESHOLD] = { 0, };

	for (i = 0; i < num_of_fwd_guessed_sboxes_2r; i++)
	{
		sequence[i] = comp_arry_2r_fwd[i].value;
	}

	for (i = 0; i < num_of_fwd_guessed_sboxes_1r; i++)
	{
		sequence[i + num_of_fwd_guessed_sboxes_2r] = comp_arry_1r_fwd[i].value;
	}

	for (i = 0; i < num_of_bwd_guessed_sboxes_2r; i++)
	{
		sequence[num_of_fwd_guessed_sboxes_2r + num_of_fwd_guessed_sboxes_1r + i] = comp_arry_2r_bwd[i].value;
	}

	for (i = 0; i < num_of_bwd_guessed_sboxes_1r; i++)
	{
		sequence[num_of_fwd_guessed_sboxes_2r + num_of_fwd_guessed_sboxes_1r + num_of_bwd_guessed_sboxes_2r + i] = comp_arry_1r_bwd[i].value;
	}
